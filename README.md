...
- Write a class method, `.all`, that returns the `@@all` variable. From inside
  the `Dog` class, we can access the `@@all` class variable, but whenever we
  might be interacting with our `Dog` class _from the outside_, this `.all` class
  method acts as our direct interface to the `@@all` variable.
